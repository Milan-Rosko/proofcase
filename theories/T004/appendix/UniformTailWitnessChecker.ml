...
let uniform_tail_window_as_nat candidate extra t lag =
  canonical_window_as_nat (add candidate.utwc_radius extra)
    (add (add candidate.utwc_start t) lag)

(** val uniform_tail_witness_matches :
    uniformTailWitnessCandidate -> nat -> nat -> bool **)

let uniform_tail_witness_matches candidate extra t =
  nat_list_eqb (uniform_tail_window_as_nat candidate extra t O)
    (uniform_tail_window_as_nat candidate extra t candidate.utwc_period)

(** val uniform_tail_witness_rejection :
    uniformTailWitnessCandidate -> nat -> nat -> uniformTailWitnessCheck **)

let uniform_tail_witness_rejection candidate extra t =
  Utwc_rejected (extra, t, (uniform_tail_window_as_nat candidate extra t O),
    (uniform_tail_window_as_nat candidate extra t candidate.utwc_period))

(** val first_uniform_tail_time_failure :
    uniformTailWitnessCandidate -> nat -> nat -> nat ->
    uniformTailWitnessCheck option **)

let rec first_uniform_tail_time_failure candidate extra t fuel =
  match uniform_tail_witness_matches candidate extra t with
  | True ->
    (match fuel with
     | O -> None
     | S fuel' -> first_uniform_tail_time_failure candidate extra (S t) fuel')
  | False -> Some (uniform_tail_witness_rejection candidate extra t)

(** val first_uniform_tail_extra_failure :
    uniformTailWitnessCandidate -> nat -> nat -> uniformTailWitnessCheck
    option **)

let rec first_uniform_tail_extra_failure candidate extra fuel =
  match first_uniform_tail_time_failure candidate extra O
          candidate.utwc_time_bound with
  | Some failure -> Some failure
  | None ->
    (match fuel with
     | O -> None
     | S fuel' -> first_uniform_tail_extra_failure candidate (S extra) fuel')

(** val check_uniform_tail_witness_candidate :
    uniformTailWitnessCandidate -> uniformTailWitnessCheck **)

let check_uniform_tail_witness_candidate candidate =
  match Nat.eqb candidate.utwc_period O with
  | True -> Utwc_invalid_period
  | False ->
    (match first_uniform_tail_extra_failure candidate O
             candidate.utwc_extra_bound with
     | Some failure -> failure
     | None ->
       Utwc_accepts_through (candidate.utwc_extra_bound,
         candidate.utwc_time_bound))
