...
| Nil -> a0
| Cons (b0, t) -> f0 b0 (fold_right f0 a0 t)
...
| Const _ -> O
| Var _ -> S O
| Add (a, b0) -> Nat.max (degree a) (degree b0)
| Sub (a, b0) -> Nat.max (degree a) (degree b0)
| Mul (a, b0) -> add (degree a) (degree b0)

(** val max_list : nat list -> nat **)

let max_list xs =
  fold_right Nat.max O xs

(** val beta_bound : nat list -> nat **)

let beta_bound a =
  S (Nat.max (length a) (max_list a))

(** val beta_d : nat list -> nat **)
    ...
    (match k' with
     ...
     | S _ ->
       (match prev_taken with
        ...
        | False ->
          (match Nat.leb (f k) rem with
           | True ->
             let pr = zeck_greedy_down k' (sub rem (f k)) True in
...
(** val enc_form : form -> nat **)

let rec enc_form = function
| F_Bot -> O
| F_Imp (a, b0) -> S (pair p0 (enc_form a) (enc_form b0))
...
let canonical_assignment pf target =
  { as_pf = pf; as_target = target; as_c = O; as_d =
    (beta_d (map enc_form pf)) }
